...
@interface UIView (JZConstraintBasedLayoutExtensions)

- (NSLayoutConstraint *)equallyRelatedConstraintWithView:(UIView *)view attribute:(NSLayoutAttribute)attribute;

@end

@interface JZWeakObjectContainer : NSObject
...
@interface JZEmptyDataSetView : UIView
...
- (id<JZEmptyDataSetSource>)emptyDataSetSource
{
    JZWeakObjectContainer *container = objc_getAssociatedObject(self, kEmptyDataSetSource);
    return container.weakObject;
}

- (id<JZEmptyDataSetDelegate>)emptyDataSetDelegate
{
    JZWeakObjectContainer *container = objc_getAssociatedObject(self, kEmptyDataSetDelegate);
    return container.weakObject;
}
...
- (JZEmptyDataSetView *)emptyDataSetView
{
    JZEmptyDataSetView *view = objc_getAssociatedObject(self, kEmptyDataSetView);
    
    if (!view)
    {
        view = [JZEmptyDataSetView new];
...
- (BOOL)jz_canDisplay
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource conformsToProtocol:@protocol(JZEmptyDataSetSource)]) {
...
- (void)setEmptyDataSetSource:(id<JZEmptyDataSetSource>)datasource
{
    if (!datasource || ![self jz_canDisplay]) {
        [self jz_invalidate];
    }
    
    objc_setAssociatedObject(self, kEmptyDataSetSource, [[JZWeakObjectContainer alloc] initWithWeakObject:datasource], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)setEmptyDataSetDelegate:(id<JZEmptyDataSetDelegate>)delegate
{
    if (!delegate) {
        [self jz_invalidate];
    }
    
    objc_setAssociatedObject(self, kEmptyDataSetDelegate, [[JZWeakObjectContainer alloc] initWithWeakObject:delegate], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}


#pragma mark - Setters (Private)

- (void)setEmptyDataSetView:(JZEmptyDataSetView *)view
{
...
- (void)jz_reloadEmptyDataSet
{
    ...
    if (([self jz_shouldDisplay] && [self jz_itemsCount] == 0) || [self jz_shouldBeForcedToDisplay])
    {
        ...
        JZEmptyDataSetView *view = self.emptyDataSetView;
...
static NSString *const JZSwizzleInfoPointerKey = @"pointer";
static NSString *const JZSwizzleInfoOwnerKey = @"owner";
static NSString *const JZSwizzleInfoSelectorKey = @"selector";
...
void jz_original_implementation(id self, SEL _cmd)
{
    ...
    NSValue *impValue = [swizzleInfo valueForKey:JZSwizzleInfoPointerKey];
...
- (void)swizzleIfPossible:(SEL)selector
{
    ...
    for (NSDictionary *info in [_impLookupTable allValues]) {
        Class class = [info objectForKey:JZSwizzleInfoOwnerKey];
        NSString *selectorName = [info objectForKey:JZSwizzleInfoSelectorKey];
    ...
    NSValue *impValue = [[_impLookupTable objectForKey:key] valueForKey:JZSwizzleInfoPointerKey];
    ...
    NSDictionary *swizzledInfo = @{JZSwizzleInfoOwnerKey: baseClass,
                                   JZSwizzleInfoSelectorKey: NSStringFromSelector(selector),
                                   JZSwizzleInfoPointerKey: [NSValue valueWithPointer:jz_newImplementation]};
...
#pragma mark - JZEmptyDataSetView

@interface JZEmptyDataSetView ()
@end

@implementation JZEmptyDataSetView
...
#pragma mark - UIView+JZConstraintBasedLayoutExtensions

@implementation UIView (JZConstraintBasedLayoutExtensions)
...
#pragma mark - JZWeakObjectContainer

@implementation JZWeakObjectContainer
...
- (JZScrollViewDataSetType)dataSetType {
    return [objc_getAssociatedObject(self, _cmd) intValue];
}

@end

@implementation JZScrollViewDataSetConfig
///默认配置
+ (instancetype)defaultConfig {
    return [JZScrollViewDataSetConfig new];
}
...
@interface JZScrollViewDataSet()<JZEmptyDataSetSource,JZEmptyDataSetDelegate>
...
#pragma mark - JZEmptyDataSetSource
- (UIView *)customViewForEmptyDataSet:(UIScrollView *)scrollView {
    return [self customView];
}
#pragma mark - JZEmptyDataSetDelegate
...
- (BOOL)emptyDataSetShouldDisplay:(UIScrollView *)scrollView {
    JZScrollViewDataSetType dataSetType = self.scrollView.dataSetType;
    if (dataSetType == JZScrollViewDataSetTypeHidden) {
        return NO;
    }
    if (dataSetType == JZScrollViewDataSetTypeError) {
        return self.errorView != nil;
    }
    if (dataSetType == JZScrollViewDataSetTypeEmpty) {
...
- (UIView *)customView {
    ...
    }else if (type == JZScrollViewDataSetTypeError) {
